...
public class MobitruUrlPropertiesValidator implements MobitruPropertiesValidator
{
    ...
    {
        Map<String, String> properties = propertyParser.getPropertyValuesByPrefix("selenium.grid.");
        ...
        String authorityFromUrl = URI.create(urlString).getAuthority();
        String authority = properties.get("username") + ":" + properties.get("password") + "@" + host;
        if (authorityFromUrl == null || !authorityFromUrl.contains(authority))
        {
            throw new IllegalStateException(String.format(INCORRECT_PROPERTY_FOR_MOBITRU_MESSAGE, URL, urlString));
        }
    }
}
